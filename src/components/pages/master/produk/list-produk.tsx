...
function ListProduk({ eventMessage, eventSocket }: Props) {
  ...
  const fetchData = async (database: string) => {
    ...
    setLoader(true)
    try {
      const response = await axios.post('https://api.synchronice.id/admin/master/produk/data', {
        kode: "",
        tahun: database
      });
  ...
  const columns: ColumnDef<UserListProps>[] = [
    {
      ...
      cell: ({ row }) => {
        return (
          <div className="text-sm">{row.getValue("kode_produk")}</div>
        )
      },
    },
    {
      ...
      header: "Nama Produk",
      cell: ({ row }) => (
        <div className="text-sm">{row.getValue("nama_produk")}</div>
      ),
    },
    {
      accessorKey: "tahun",
  ...
  const exportToExcel = () => {
    const exampleKeys = ['id', 'kode_produk', 'nama_produk', 'tahun'];
    ...
    XLSX.utils.sheet_add_aoa(standarSheet, [
      ['COLUMN', 'KETERANGAN'],
      ['kode_produk', 'kode produk sama dengan report f5/f6'],
      ['nama_produk', 'nama produk sama dengan report f5/f6'],
    ...
    XLSX.writeFile(workbook, `standar-data-${formatType}-produk.xlsx`);
  };
