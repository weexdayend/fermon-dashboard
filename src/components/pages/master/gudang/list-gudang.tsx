...
function ListGudang({ eventMessage, eventSocket }: Props) {
  ...
  const fetchData = async (database: string) => {
    ...
    setLoader(true)
    try {
      const response = await axios.post('https://api.synchronice.id/gudang/id', {
        kode: "",
        tahun: database
      });
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.sheet_add_aoa(standarSheet, [
      ['COLUMN', 'KETERANGAN'],
      ['kode_gudang', 'kode gudang sama dengan data gudang'],
      ['nama_gudang', 'nama gudang sama dengan data gudang'],
      ['alamat', 'alamat lengkap gudang'],
      ['long', 'longitude gudang'],
      ['lat', 'lattitude gudang'],
  ]);
    ...
    XLSX.writeFile(workbook, `standar-data-${formatType}-gudang.xlsx`);
  };
