...
function ListAlokasi({ eventSocket, eventMessage }: Props) {
  ...
  const exportToExcel = () => {
    const exampleKeys = ['id', 'kode', 'kode_produk', 'besaran', 'bulan', 'tahun', 'kategori'];
    ...
    XLSX.utils.sheet_add_aoa(standarSheet, [
      ['COLUMN', 'KETERANGAN'],
      ['kode', 'isi dengan kode kecamatan'],
      ...
      ['kategori', 'isi dengan kategori yang ingin anda input', 'Kecamatan, Gudang, Distributor, Kios'],
    ]);
    ...
    XLSX.writeFile(workbook, `standar-data-${formatType}-alokasi.xlsx`);
  };
