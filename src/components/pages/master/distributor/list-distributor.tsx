...
function ListDistributor({ eventMessage, eventSocket, role }: Props) {
  ...
  const [kodeDistributor, setKodeDistributor] = useState<string>('')
  const [namaDistributor, setNamaDistributor] = useState<string>('')
  const [alamatDistributor, setAlamatDistributor] = useState<string>('')
  const [longitudeDistributor, setLongitudeDistributor] = useState<string>('')
  const [lattitudeDistributor, setLattitudeDistributor] = useState<string>('')

  const handleEditClick = (index: number, data: UserListProps) => {
    setEditIndex(index)
    setEditData(true)

    setIdDistributor(data.id)
    setKodeDistributor(data.kode_distributor)
    setNamaDistributor(data.nama_distributor)
    setAlamatDistributor(data.alamat)
    setLongitudeDistributor(data.long)
    setLattitudeDistributor(data.lat)
  };

  const handleSubmitEdit = async () => {
    setLoader(true)
    try {
      await axios.put('https://api.greatjbb.com/distributor', {
        id: idDistributor,
        kode: kodeDistributor,
        nama_gudang: namaDistributor,
        alamat: alamatDistributor,
        long: longitudeDistributor,
        lat: lattitudeDistributor,
      });
    } catch (error) {
  ...
  const exportToExcel = () => {
    const exampleKeys = ['id', 'kode_distributor', 'nama_distributor', 'alamat', 'long', 'lat', 'tahun'];
    ...
    XLSX.utils.sheet_add_aoa(standarSheet, [
      ['COLUMN', 'KETERANGAN'],
      ['kode_distributor', 'kode distributor sama dengan report f5'],
      ['nama_distributor', 'nama distributor sama dengan report f5'],
      ['alamat', 'alamat lengkap distributor'],
      ['long', 'longitude distributor'],
      ['lat', 'lattitude distributor'],
  ]);
    ...
    XLSX.writeFile(workbook, `standar-data-${formatType}-distributor.xlsx`);
  };
  ...
  const tabIdentifier = 'Distributor'
                          ...
                          (
                            <>
                              <div className='flex flex-col w-full gap-4'>
                                <div className='flex flex-col gap-1'>
                                  <Label htmlFor='kode_gudang'>Kode Distributor</Label>
                                  <Input id='kode_gudang' value={kodeDistributor} onChange={(e) => setKodeDistributor(e.target.value)} />
                                </div>
                                <div className='flex flex-col gap-1'>
                                  <Label htmlFor='nama_gudang'>Nama Distributor</Label>
                                  <Input id='nama_gudang' value={namaDistributor} onChange={(e) => setNamaDistributor(e.target.value)} />
                                </div>
                                <div className='flex flex-col gap-1'>
                                  <Label htmlFor='alamat_gudang'>Alamat Distributor</Label>
                                  <Input id='alamat_gudang' value={alamatDistributor} onChange={(e) => setAlamatDistributor(e.target.value)} />
                                </div>
                              </div>
                              <div className='flex flex-col w-full gap-4'>
                                <div className='flex flex-col gap-1'>
                                  <Label htmlFor='long_gudang'>Longitude</Label>
                                  <Input id='long_gudang' value={longitudeDistributor} onChange={(e) => setLongitudeDistributor(e.target.value)} />
                                </div>
                                <div className='flex flex-col gap-1'>
                                  <Label htmlFor='lat_gudang'>Lattitude</Label>
                                  <Input id='lat_gudang' value={lattitudeDistributor} onChange={(e) => setLattitudeDistributor(e.target.value)} />
                                </div>
                              </div>
                            </>
                          ) : (
                            <>
                              <div className='flex flex-col w-full'>
                                <h1 className='text-xs opacity-70'>{item.kode_distributor}</h1>
                                <h1 className='text-sm'>{item.nama_distributor}</h1>
                              </div>
                              <div className='flex flex-col w-full'>
                                        ...
                                        file.uri === null ? (
                                          <Dialog key={idxFile}>
                                            ...
                                            <DialogContent className="sm:max-w-[425px]">
                                              ...
                                              <DialogFooter>
                                                <Button onClick={() => handleUploadFile(file.kategori, item.kode_distributor)} type="submit">Upload file</Button>
                                              </DialogFooter>
                                            </DialogContent>
                                          </Dialog>
                                        ) : (
